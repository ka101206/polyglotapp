    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      ...
      >
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-100 dark:bg-slate-800">
          <h2 className="text-xl font-bold flex items-center"><InboxIcon className="mr-2"/> {t('inbox')}</h2>
          <button onClick={onClose} className="text-slate-500 hover:text-slate-800 dark:hover:text-slate-200"><X /></button>
        </div>
        
        <div className="p-4 max-h-[60vh] overflow-y-auto">
          {loading ? (
            <p className="text-center text-slate-500 py-4">{t('loadingInvites')}</p>
          ) : invites.length === 0 ? (
            ...
            <div className="space-y-4">
              {invites.map(invite => (
                <div key={invite.id} className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                  <p className="font-semibold text-slate-800 dark:text-slate-200">{t('invitedYouToJoin', { admin: invite.admin_name })}</p>
                  ...
                  <div className="flex gap-2 mt-4">
                    ...
                    >
                      <Check size={18} className="mr-1" /> {t('accept')}
                    </button>
                    <button 
                    ...
                    >
                      <X size={18} className="mr-1" /> {t('decline')}
                    </button>
                  </div>
                </div>
              ))}
